<template>
  <div
    class="h-screen max-w-md mx-auto flex justify-center relative management-guide-line"
  >
    <div class="wrap relative">
      <div class="management-detail-content">
        <div class="management-detail-top">
          <div class="title-wrap">
            <router-link to="/management/index">
              <svg width="32" height="32" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6l6 6l1.41-1.41z"
                />
              </svg>
            </router-link>
            <h2>Weight</h2>
          </div>
          <ul class="common-style-tab">
            <li class="active"><a href="">Daily</a></li>
            <li><a href="">Weekly</a></li>
            <li><a href="">Monthly</a></li>
          </ul>
          <div class="date-slide">
            <button>
              <svg width="32" height="32" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6l6 6l1.41-1.41z"
                />
              </svg>
            </button>
            <h3>
              14, Jan
              <a href=""
                ><svg width="20" height="20" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
                    d="m12 15.4l-6-6L7.4 8l4.6 4.6L16.6 8L18 9.4Z"
                  /></svg
              ></a>
            </h3>
            <button>
              <svg width="32" height="32" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M8.59 16.59L13.17 12L8.59 7.41L10 6l6 6l-6 6l-1.41-1.41z"
                />
              </svg>
            </button>
          </div>
        </div>
        <div class="management-detail-bottom">
          <div class="item-data-weight">
            <div class="time">
              <strong>13:48</strong>
              <span>PM</span>
            </div>
            <div class="graph"></div>
            <strong class="weight"> 65.2 </strong>
          </div>
          <div class="item-data-weight">
            <div class="time">
              <strong>13:48</strong>
              <span>PM</span>
            </div>
            <div class="graph"></div>
            <strong class="weight"> 65.2 </strong>
          </div>
          <div class="item-data-weight">
            <div class="time">
              <strong>13:48</strong>
              <span>PM</span>
            </div>
            <div class="graph"></div>
            <strong class="weight"> 65.2 </strong>
          </div>
        </div>
      </div>
    </div>
    <div class="weight-unit rounded-full">
      <button class="active" @click="$router.push('/')">Lb</button>
      <button class="" @click="$router.push('/')">Kg</button>
    </div>
  </div>
</template>

<style lang="scss">
.management-guide-line {
  padding: 2rem 0 1rem;
  border: 1px solid #eee;
  flex-wrap: wrap;
  background: #f7fbff;
}
.wrap {
  width: 85%;
  margin: 0 auto;
}
.management-detail-content {
  //  width: 80%;
  // margin: 0 auto;
  h2 {
    font-size: 28px;
    font-weight: bold;
    color: #3d95ff;
    text-align: center;
  }
  .management-detail-top {
    .title-wrap {
      position: relative;
      padding: 3px 0 16px;
      a {
        display: flex;
        justify-content: center;
        align-items: center;
        width: 25px;
        height: 25px;
        border-radius: 50%;
        background-color: #fff;
        box-shadow: 0 0 6px 0 rgba(0, 0, 0, 0.3);
        position: absolute;
        left: 0;
        top: 0;
        z-index: 10;
        svg {
          color: #3d95ff;
        }
      }
      h2 {
        font-size: 18px;
        font-weight: bold;
        line-height: 1.18;
        color: #555;
      }
    }
    .common-style-tab {
      width: 100%;
      height: 46px;
      margin: 0 auto;
      border-radius: 10px;
      box-shadow: 0 0 10px 0 rgba(6, 44, 90, 0.12);
      display: flex;
      justify-content: center;
      align-items: center;
      li {
        width: 33.333%;
        position: relative;

        &:after {
          content: '';
          width: 1px;
          height: 15px;
          background: #ddd;
          margin-top: 4px;
          position: absolute;
          top: 0;
          right: 0;
        }
        &:last-child {
          &:after {
            content: none;
          }
        }
        a {
          display: flex;
          align-items: center;
          font-size: 13px;
          font-weight: 500;
          color: #555;
          justify-content: center;
          padding: 0 9px;
        }
        &.active {
          &:after {
            content: none;
          }
          a {
            width: 83%;
            border-radius: 10px;
            background-color: #3d95ff;
            color: #fff;
            height: 36px;
            margin: 0 auto;
          }
        }
      }
    }
    .date-slide {
      display: flex;
      justify-content: space-between;
      align-items: center;
      width: 100%;
      height: 75px;
      border-radius: 15px;
      background: #fff;
      margin-top: 15px;
      box-shadow: 0 0 10px 0 rgb(6 44 90 / 12%);
      padding: 0 1rem;
      button {
        width: 25px;
        height: 25px;
        border-radius: 50%;
        display: flex;
        justify-content: center;
        align-items: center;
        background-color: #d6e9ff;
        > svg {
          color: #3c95ff;
        }
      }
      h3 {
        font-size: 20px;
        font-weight: bold;
        line-height: 1.15;
        color: #555;
        text-align: center;
        position: relative;
        a {
          position: absolute;
          left: 50%;
          margin-left: -10px;
          bottom: -20px;
          display: block;
          width: 20px;
          svg {
            font-size: 11px;
          }
        }
      }
    }
  }
  .management-detail-bottom {
    padding: 1rem 0;
    .item-data-weight {
      margin-bottom: 1rem;
      min-height: 90px;
      border-radius: 15px;
      box-shadow: 0 0 10px 0 rgba(6, 44, 90, 0.12);
      background-color: #fff;
      display: flex;
      justify-content: space-between;
      align-items: center;
      padding: 0 23px;
      .time {
        display: flex;
        align-items: center;
        strong {
          font-size: 20px;
          font-weight: bold;
          line-height: 1.18;
          color: #555;
        }
        span {
          display: block;
          font-size: 14px;
          font-weight: 500;
          line-height: 1.19;
          color: #555;
          margin-left: 10px;
        }
      }
      .weight {
        font-size: 26px;
        font-weight: bold;
        line-height: 1.14;
        color: #3d95ff;
      }
    }
  }
}

.weight-unit {
  border: 1px solid #3c95ff;
  display: flex;
  justify-content: center;
  align-items: center;
  height: 50px;
  overflow: hidden;
  margin-top: auto;
  margin-left: auto;
  margin-right: 7%;
  button {
    height: 100%;
    width: 50px;
    color: #555;

    &.active {
      background: #3c95ff;
      color: #fff;
    }
  }
}
</style>
